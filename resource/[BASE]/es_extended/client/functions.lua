...
ESX.ShowInventory = function()
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.count > 0 then
			table.insert(elements, {
				icon      = 'https://cfx-nui-szymczakovv_binds/html/img/' .. v.name .. '.png', --to masz dodac zeby ikonki byly es? (ZMIEŃ NAZWE "TWOJSKRYPT" NA NA NAZWĘ SKRYPTU W KTORYM BEDA IKONKI)
				label = ((v.count > 1 and ('[%s/%s] %s'):format(v.count, v.limit, v.label) or v.label)),
				count = v.count,
				type = 'item_standard',
				value = v.name,
				usable = v.usable,
		...
		table.insert(elements, {label = "[Zarzadznie Bindami]", action = 'unbind'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		...
		if data.current.usable or CheckWeapons(data.current.value) then
			table.insert(elements, {label = "Przypisz [1-5]", action = 'bind', type = data.current.type, value = data.current.value, number = data.current.number})
		end
...
openBindsMenu = function(datavalue)
    ...
    ESX.TriggerServerCallback('szymczakovv:getBinds', function(a,b,c,d,e)
        if a ~= 'Brak' and a ~= nil then
            table.insert(elements, { label = '[1] Naćisnij aby usunąć - '..tostring(a), what = 'first' })
        else
            table.insert(elements, { label = '[1] Brak przypisanego klawisza pod item', what = 'first' })
        end
        if b ~= 'Brak' and b ~= nil then
            table.insert(elements, { label = '[2] Naćisnij aby usunąć - '..tostring(b), what = 'second' })
        else
            table.insert(elements, { label = '[2] Brak przypisanego klawisza pod item', what = 'second' })
        end
        if c ~= 'Brak' and c ~= nil then
            table.insert(elements, { label = '[3] Naćisnij aby usunąć - '..tostring(c), what = 'third' })
        else
            table.insert(elements, { label = '[3] Brak przypisanego klawisza pod item', what = 'third' })
        end
        if d ~= 'Brak' and d ~= nil then
            table.insert(elements, { label = '[4] Naćisnij aby usunąć - '..tostring(d), what = 'fourth' })
        else
            table.insert(elements, { label = '[4] Brak przypisanego klawisza pod item', what = 'fourth'})
        end
        if e ~= 'Brak' and e ~= nil then
            table.insert(elements, { label = '[5] Naćisnij aby usunąć - '..tostring(e), what = 'fifth'})
        else
            table.insert(elements, { label = '[5] Brak przypisanego klawisza pod item', what = 'fifth'})
        end
    end)
...
RegisterNetEvent('esx:Items')
AddEventHandler('esx:Items', function()
    ESX.TriggerServerCallback('esx:loadItems', function(data) 
        ESX.Items = data
    end)
end)
...
function xd()
	...
	TriggerServerEvent('ChujCieToWSM')
	xd()
end