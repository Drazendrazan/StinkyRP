local WebhookEasyAdmin = 'http://logselitav3.egghost.pl/api/webhooks/892106558396305468/RpnxNvrrLhuEiIIXanC5kDSh47H9FKgBbZ77_oXsaPb-jMGjdImkZNxJKcmvxD2hdTWx'
local EasyAdminReportWebhook = 'http://logselitav3.egghost.pl/api/webhooks/892106779159310398/marR8ILYl0ioNsWHOJoy0KZxXyi2FTEITh-I85-zyW_vUmxwle7K-c99jtjsUy4AStut'
local blockReport = {}
...
Citizen.CreateThread(function()
	...
	--[[RegisterCommand("report", function(source, args, rawCommand)
		if not blockReport[source] then
			...
			--local _source = source
			--local xPlayer = ESX.GetPlayerFromId(_source)
			--if xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'best' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.group == 'trialsupport' then

			for i,_ in pairs(OnlineAdmins) do 
				TriggerClientEvent('chatMessage', i,  "\n^*^1 REPORT: [^*^3"  ..source.. " ^*^1| ^*^3" .. name .."^*^1] : ^*^3" .. reason)
			end
			...
			local steamid = xPlayer.identifier
			local name = GetPlayerName(source)
			wiadomosc = "Report: "..reason.." \n[ID: "..source.." | Nazwa Steam: "..name.." | ROCKSTAR: "..steamid.." ]" 
			DiscordHookReport('EasyAdmin', wiadomosc, 11750815)
			TriggerClientEvent('chatMessage', source, "^*^1 REPORT: ^*^3 ZOSTAŁ WYSŁANY DO ^*^1^*^3ADMINISTRACJI")

			Wait(30000)

			blockReport[source] = false
		else
			local xPlayer = ESX.GetPlayerFromId(source)
			TriggerClientEvent('chatMessage', source, "^1[BŁĄD]", {255,255,255}, 'Reporty możesz wysyłać co 30 sekund!')
		end
	
	end, false)

	function DiscordHookReport(hook,message,color)
		local embeds = {
					{
				["title"] = message,
				["type"] = "rich",
		...
		PerformHttpRequest(EasyAdminReportWebhook, function(err, text, headers) end, 'POST', json.encode({ username = hook,embeds = embeds}), { ['Content-Type'] = 'application/json' })
	end--]]
	    	...
	    	if args[1] and tonumber(args[1]) then
	      		...
	      		if xTarget then
	        		...
	        		xTarget.setCoords(playerCoords)
						xPlayer.showNotification('~o~Teleportujesz do siebie: '..args[1])
	      		else
					xPlayer.showNotification('~o~Gracz nie jest online')
	      		end
	    	else
				xPlayer.showNotification('~o~Źle wpisana komenda')
	    	end
		else
		...
		if xPlayer.group == 'best' or xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'mod' or xPlayer.group == 'support' then
    		if args[1] and tonumber(args[1]) then
      			...
      			if xTarget then
					...
					xPlayer.showNotification('~o~Teleportujesz się siebie: '..args[1])
      			else
					xPlayer.showNotification('~o~Gracz nie jest online')
      			end
    		else
    		...
    		if args[1] and tonumber(args[1]) then
      			...
      			if xTarget then
        			...
        			if playerCoords then
          			xTarget.setCoords(playerCoords)
					 xPlayer.showNotification('~o~Przeteleportowałeś gracza do ostatniej pozycji: '..args[1])
          			savedCoords[targetId] = nil
        		else
					xPlayer.showNotification('~o~Nie ma gdzie przeteleportować gracza!')
        			end
      			else
					xPlayer.showNotification('~o~Gracz nie jest online')
      			end
	...
	if source ~= 0 then
		...
		if xPlayer.group == 'best' or xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'mod' or xPlayer.group == 'support' then
	    	...
	    	if playerCoords then
				...
				xPlayer.showNotification('~o~Wróciłeś do ostatniej pozycji')
	      		savedCoords[source] = nil
	    	else
				xPlayer.showNotification('~o~Nie ma gdzie przeteleportować!')
	    	end
	  	end
	end
end, false)

RegisterServerEvent("Stinky:EasyAdminLog")
AddEventHandler("Stinky:EasyAdminLog", function(what)
    local xPlayer = ESX.GetPlayerFromId(source)
    local steamid = xPlayer.identifier
    local name = GetPlayerName(source)
    wiadomosc = ""..what.."\n[ID: "..source.." | Nazwa Steam: "..name.." | ROCKSTAR: "..steamid.." ]" 
...
function EasyAdmin(hook,message,color)
	...
	if message == nil or message == '' then return FALSE end
	PerformHttpRequest(WebhookEasyAdmin, function(err, text, headers) end, 'POST', json.encode({ username = hook,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end