...
RegisterKeyMapping('+easyadmin', 'Menu Administatora', 'keyboard', 'F10')
...
Citizen.CreateThread(function()
	...
	local subtitle = "~b~EasyAdmin"
	-- if settings.updateAvailable then
	-- 	subtitle = "~g~UPDATE "..settings.updateAvailable.." AVAILABLE!"
	-- end

	mainMenu = NativeUI.CreateMenu("EasyAdmin", "EasyAdmin", menuOrientation, 0)
...
function GenerateMenu() -- this is a big ass function
	...
	mainMenu = NativeUI.CreateMenu("EasyAdmin", "~y~ Menu", menuOrientation, 0)
	_menuPool:Add(mainMenu)
	...
	for i,thePlayer in pairs(players) do
		...
		if permissions["spectate"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent('move_logs:triggerLog', 'Spectuje gracza ['.. thePlayer.id ..'] '.. thePlayer.name ..'', 10181046, 'https://discord.com/api/webhooks/962036891727826954/wiDZP9woQhAs4gZfDbcUeE0CMCtvNKrjyg-NJVGMwKZvsmwbU-abgUOXVLvz3M01NACK')
			end
		end
		...
		if permissions["teleport.player"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				if not RedM then
					...
					TriggerServerEvent('move_logs:triggerLog', 'Teleportował sie do gracza ['.. thePlayer.id..'] '.. thePlayer.name..'', 10181046, 'https://discord.com/api/webhooks/962037201661735062/j3IVFiIQPyPLSj1dEMxU8J1e62MeNKLBHOynMhIgS1TU5y4UQNo51QcSZNHDeSECyy2w')
				else
					...
					--TriggerServerEvent('move_logs:triggerLog', 'Teleportował sie do gracza ['.. thePlayer.id..'] '.. thePlayer.name..'', 10181046, 'https://discord.com/api/webhooks/930492684869730316/nrnXT_neb9PEcknBFZeWpOce6ZnqY4lD2wDvVPOmQknZw1NK5Fo602Vq5VwOctulN3nN')
				end
			end
		end
		...
		if permissions["teleport.player"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent('move_logs:triggerLog', 'Teleportowal gracza do siebie ['.. thePlayer.id ..'] '.. thePlayer.name ..' KORDY '.. coords ..'', 10181046, 'https://discord.com/api/webhooks/962037201661735062/j3IVFiIQPyPLSj1dEMxU8J1e62MeNKLBHOynMhIgS1TU5y4UQNo51QcSZNHDeSECyy2w')
			end
		end
		...
		if permissions["slap"] then
			...
			thisItem.OnSliderSelected = function(index)
				...
				TriggerServerEvent('move_logs:triggerLog', 'Dal Slaya graczowi ['.. thePlayer.id..'] '.. thePlayer.name..'', 10181046, 'https://discord.com/api/webhooks/962037352321155133/MXkAK-jbRyyyrV3atYqL1FyrX9jbwiHpD9ZsQJzKeiGZQNsJJW6pwRdMBeKiN33czT5_')
			end
		end
			...
			thisPlayer.OnListSelect = function(sender, item, index)
					if item == thisItem then
							...
							if i == GetLocalisedText("on") then
								TriggerServerEvent('move_logs:triggerLog', 'Dal Freeza graczowi ['.. thePlayer.id..'] '.. thePlayer.name..'', 10181046, 'https://discord.com/api/webhooks/962037491588792420/aitQWmT1OXkk3ZTFUv--GajKPVIVhM8XeNGEuQF9sPufvAdHWH90aHV_CvG8RnHZsiAM')
								TriggerServerEvent("EasyAdmin:FreezePlayer", thePlayer.id, true)
							else
								TriggerServerEvent('move_logs:triggerLog', 'Zabrał Freeza graczowi ['.. thePlayer.id..'] '.. thePlayer.name..'', 10181046, 'https://discord.com/api/webhooks/962037491588792420/aitQWmT1OXkk3ZTFUv--GajKPVIVhM8XeNGEuQF9sPufvAdHWH90aHV_CvG8RnHZsiAM')
	...
	function tpm()
		...
		if DoesBlipExist(WaypointHandle) then
			...
			TriggerServerEvent('move_logs:triggerLog', 'Przeteleportował sie poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962037201661735062/j3IVFiIQPyPLSj1dEMxU8J1e62MeNKLBHOynMhIgS1TU5y4UQNo51QcSZNHDeSECyy2w')
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent('move_logs:triggerLog', 'Naprawil Pojazd poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962037870598717480/qtyA1rHbGQXPvDahZ4ANo-00_E4LNO-LY-gd3p8jeqv0SUj2a9Yovk1bboVf5y1AONIy')
			TriggerServerEvent("Stinky:EasyAdminLog", what)
		end
	end
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			daojosdinpatpemata()
			what = "obrócił pojazd"
			TriggerServerEvent('move_logs:triggerLog', 'Obrócił Pojazd przy pomocy EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962037870598717480/qtyA1rHbGQXPvDahZ4ANo-00_E4LNO-LY-gd3p8jeqv0SUj2a9Yovk1bboVf5y1AONIy')
			TriggerServerEvent("Stinky:EasyAdminLog", what)
		end
	end
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent('move_logs:triggerLog', 'Usunał pojazd przy pomocy EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962037870598717480/qtyA1rHbGQXPvDahZ4ANo-00_E4LNO-LY-gd3p8jeqv0SUj2a9Yovk1bboVf5y1AONIy')
		end
	end
	if permissions["ban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent('move_logs:triggerLog', 'Dodał sobie pojazd przy pomocy EasyAdmina '.. vehicle ..'', 10181046, 'https://discord.com/api/webhooks/962038176896151663/TMUtwCUbQudcPcQEv58UcInlWaU-IDWVhAlhzkELBzC1WrN2UKZIo5YoA6Ktx94h8TeT')
		end
	end
end
	...
	if permissions["ban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerEvent('esx:showNotification', 'Dodałeś armor!')
			what = "Dodano armor"
			TriggerServerEvent("Stinky:EasyAdminLog", what)
			TriggerServerEvent('move_logs:triggerLog', 'Dodał sobie armor poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962038289366409286/zbrnK4qWcFW8n2UWza8Px_N1L134Oi7ndRiQk9-J5iadfzob3DVPCkQMP5czLGRq1Hth')
		end
	end
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			what = "zabił się"
			TriggerServerEvent('move_logs:triggerLog', 'Zabił się poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962038862320926751/RXbU1Kgo4JdhgZD2f_jM8TtdHhK0g-nLpyjFUmk70RXvUmZMRcQJrjbo5gSEcFCeuCED')
			TriggerServerEvent("Stinky:EasyAdminLog", what)
		end
	end
	...
	if permissions["unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent('move_logs:triggerLog', 'Dal sobie revive poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962038636205985832/5OBLjaJWj4tFzTzPYBh11jKSjhWEcDWu6PDIGj8Wn1VJx0TcHezSrlFzPRV9frAhcJ7Z')
			TriggerServerEvent("Stinky:EasyAdminLog", what)
		end
	end
	...
	if permissions["ban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent('move_logs:triggerLog', 'Dal sobie heala poprzez EasyAdmina', 10181046, 'https://discord.com/api/webhooks/962038424750129302/qcT_-Ss8w74XrrSOaBovi29KOwxo7bmUY77jw9IvpN7oWbsSyKZ89Fth5Bp4jjSNEoxx')
			what = "Uleczył się"
			TriggerServerEvent("Stinky:EasyAdminLog", what)
		end
	end
...
if permissions["unban"] then
	local thisItem = NativeUI.CreateCheckboxItem('Nie widoczność', not IsEntityVisible(PlayerPedId()), "")
	twojstarydev:AddItem(thisItem)
	thisItem.CheckboxEvent = function(sender, item, status)
		if item == thisItem then
			...
			TriggerServerEvent("Stinky:EasyAdminLog", what)
			TriggerServerEvent('move_logs:triggerLog', 'Odpalił sobie niewidke', 10181046, 'https://discord.com/api/webhooks/962039072728166553/KBdj-Qd2eGyT58mQcM9KI78fUe-zc1iyLeIW0MbVNjuJe9LyY82yb7fuugkqUTJzKUwJ')
			else
...
--[[if permissions["ban"] then
	local thisItem = NativeUI.CreateCheckboxItem('GodMode', not IsEntityVisible(PlayerPedId()), "")
	twojstarydev:AddItem(thisItem)
	thisItem.CheckboxEvent = function(sender, item, status)
		if item == thisItem then
			local pid = PlayerPedId()
...
if permissions["unban"] then
	...
	twojstarydev:AddItem(thisItem)
	TriggerServerEvent('move_logs:triggerLog', 'Włączył sobie Noclipa', 10181046, 'https://discord.com/api/webhooks/962039190684581888/s8ay9vput2qMgLkwCYJOanurtEsYq4a8qrBs-RYvh95cuzoXp5fVijd7yazsRj_kTslD')
	thisItem.CheckboxEvent = function(sender, item, status)
		if item == thisItem then
			noClip = not noClip
			if not noClip then
...
if permissions["unban"] then
	...
	thisItem.Activated = function(ParentMenu,SelectedItem)
		tpm()
		what = "wykonał funkcję TPM"
		TriggerServerEvent("Stinky:EasyAdminLog", what)
		TriggerServerEvent('move_logs:triggerLog', 'Teleportował sie do znacznika', 10181046, 'https://discord.com/api/webhooks/962039339011944450/b-zm_P2VMDIL7i2ePYV4L2MgY3KacUkwQpUfGg1NNixf1bFsUPmNYvjheflwC3oDVKpx')
	end
end
