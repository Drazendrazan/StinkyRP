...
function OpenWeaponsMenu(station)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if data.current.value == 'withdraw' then
			ESX.TriggerServerCallback('Stinky_stocks:getStock', function(inventory)
				local elements = {}
				...
				ESX.UI.Menu.Open(
				  ...
				  function(data, menu)
					menu.close()
					TriggerServerEvent('Stinky_stocks:getWeaponInStock', data.current.value, data.current.ammo, station)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					if data.current.ammo <= 1 then
						menu.close()
						TriggerServerEvent('Stinky_stocks:putWeaponInStock', data.current.value, data.current.ammo, station)
						ESX.SetTimeout(500, function()
							OpenWeaponsMenu(station)
						end)
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
							...
							if quantity == nil then
								...
								if quantity <= data.current.ammo then
									menu.close()
									menu2.close()
									TriggerServerEvent('Stinky_stocks:putWeaponInStock', data.current.value, tonumber(data2.value), station)
									ESX.SetTimeout(500, function()
										OpenWeaponsMenu(station)
									end)
								else
		...
		if isUsing then
			isUsing = false
			TriggerServerEvent('Stinky_organizations:setStockUsed', 'society_'..PlayerData.hiddenjob.name, 'weapons', false)
		end
	end)
end
...
function OpenInventoryMenu(station)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if isUsing then
			isUsing = false
			TriggerServerEvent('Stinky_organizations:setStockUsed', 'society_'..PlayerData.hiddenjob.name, 'inventory', false)
		end
	end)
end
			...
			if IsControlJustReleased(0, 38) and PlayerData.hiddenjob and Config.Organisations[PlayerData.hiddenjob.name] and not exports['esx_policejob']:isHandcuffed() and not exports['esx_ambulancejob']:getDeathStatus() then
				if CurrentAction == 'menu_armory' then
					...
					--if closestDistance > 3 or closestPlayer == -1 then
						--ESX.TriggerServerCallback('Stinky_organizations:checkStock', function()
							--if not isUsed then
								isUsing = true
								TriggerServerEvent('Stinky_organizations:setStockUsed', 'society_'..PlayerData.hiddenjob.name, 'inventory', true)
								zoneName = 'inventory'
				...
				elseif CurrentAction == 'menu_cloakroom' then
					...
					ESX.TriggerServerCallback('Stinky_stocks:getPlayerDressing', function(dressing)
						local elements = {}
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
							...
							TriggerEvent('skinchanger:getSkin', function(skin)
								ESX.TriggerServerCallback('Stinky_stocks:getPlayerOutfit', function(clothes)
...
OpenContractMenu = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Stinky_jest_git', { title = 'Organizacja '..Config.Organisations[PlayerData.hiddenjob.name].Label, align = 'center', elements = elements}, function(data, menu) if data.current.value == Config.Organisations[PlayerData.hiddenjob.name].Contract.Utils.Price then klameczka = Config.Organisations[PlayerData.hiddenjob.name].Contract.Utils.Weapon TriggerServerEvent('Stinky_organizations', klameczka) elseif data.current.value == 'ammo' then TriggerServerEvent('Stinky_stocks:Magazynek') end
	end, function(data, menu)
		menu.close()
	end)
end
