				...
				if aiming or shooting then
					...
					if status == 1 and shooting then
						...
						if curMul > maxMul then
							local jaki = curMul
							TriggerServerEvent('Stinky:FuckniggaCheaters', jaki)
						end
					end
...
--[[
AddEventHandler('Stinky:loading', function(cb)
	cb(loadingStatus)
end)

RegisterNetEvent('esx:playerLoaded')
AddEventHandler('esx:playerLoaded', function(xPlayer)
	print('[Stinky] Postać załadowana')
	if not loadingPosition then
		...
		print('[Stinky] Przeniesiono do poczekalni')
	end
end)

AddEventHandler('skinchanger:modelLoaded', function()
	print('[Stinky] Model załadowany')
	ModelLoaded()
end)

AddEventHandler('Stinky:passthrough', function()
	print('[Stinky] Nowy gracz, ładowanie')
	ModelLoaded()
end)
...
function ModelLoaded()
	if loadingStatus < 2 then
		Citizen.CreateThreadNow(function()
			print('[Stinky] Oczekiwanie na załadowanie postaci')
			...
			print('[Stinky] Odblokowano wejście (LPM)')
		end)
	end
end
