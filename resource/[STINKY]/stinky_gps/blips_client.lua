...
RegisterNetEvent('Stinky_gps:openMenu')
AddEventHandler('Stinky_gps:openMenu', function()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gpsmen',
	{
		...
		if data2.current.value == '111' then
			menu2.close()
			TriggerServerEvent("Stinky_gps:refreshGPS")
            TriggerEvent('esx_policejob:displayGPS')
			Citizen.Wait(1500)
		elseif data2.current.value == '222' then
			menu2.close()
			TriggerServerEvent("Stinky_gps:destroyGPS")
			Citizen.Wait(1500)
		end
...
RegisterNetEvent('Stinky_gps:lostGPS')
AddEventHandler('Stinky_gps:lostGPS', function(characterInfo, coords)
