...
ESX.RegisterServerCallback('Stinky_selldrugs:checkDrugs', function(source, cb, LSPD)
...
RegisterServerEvent('Stinky_selldrugs:sell')
AddEventHandler('Stinky_selldrugs:sell', function(LSPD)
	local xPlayer = ESX.GetPlayerFromId(source)	
	xDrug = 0
	...
	local weedPrice = 2900 -- 1700
	local methPrice = 3700 -- 2900
	local heroinaPrice = 5200 -- 4200
	local cokePrice = 4550 -- 3700
	--KOKA PERICO -- 4000
	...
	if drugType ~= nil then
		...
		TriggerClientEvent('Stinky_selldrugs:stopSell', xPlayer.source, 1, ('~o~'.. xDrug ..' w ilośći '.. drugName..' za ' .. blackMoney .. "$" ))
...
RegisterServerEvent('Stinky_selldrugs:triggerDispatch')
AddEventHandler('Stinky_selldrugs:triggerDispatch', function(pos, street, photo)
	...
	for i=1, #xPlayers, 1 do
		...
		if (xPlayer.job.name == 'police') then
			...
			TriggerClientEvent('Stinky_selldrugs:showNotify', xPlayer.source, pos, street, msg)
		end
	end
end)
