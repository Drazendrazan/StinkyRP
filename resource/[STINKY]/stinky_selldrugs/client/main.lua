...
RegisterNetEvent('Stinky_selldrugs:photoMessage')
AddEventHandler('Stinky_selldrugs:photoMessage', function(id)
  local myId = PlayerId()
...
Citizen.CreateThread(function()
	print('[Stinky-DRUGS] - Starting main courtine')
	while true do
		Citizen.Wait(5)
											...
											if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(_pedHandle), true) < 0.8 then
												if (ScriptData.lastPed ~= _pedHandle) then
													...
													if (ScriptData.hasDrugs == true) and (ScriptData.lspdCount >= 1 ) then
														ESX.TriggerServerCallback('Stinky_selldrugs:checkDrugs', function(cb)
															if cb == 1 then
...
SellDrugs = function(targetPed, lspdCount)
	...
	TriggerServerEvent('Stinky_selldrugs:sell', lspdCount)
end
...
FailSellDrugs = function(targetPed)
	...
	ScriptData.lastPed = targetPed
	TriggerEvent('Stinky_selldrugs:stopSell', 2)
end
...
RegisterNetEvent('Stinky_selldrugs:stopSell')
AddEventHandler('Stinky_selldrugs:stopSell', function(soldInt, soldString)
	local playerPed = PlayerPedId()
	...
	elseif soldInt == 2 then
		...
		if notify == 1 then
			...
			TriggerEvent('Stinky_selldrugs:photoMessage', GetPlayerServerId(PlayerId()))
			TriggerServerEvent('Stinky_selldrugs:triggerDispatch', coords, street, true)
			ESX.ShowNotification('~b~Osoba nie jest zainteresowana')
		elseif notify == 2 then
			local coords = GetEntityCoords(playerPed)
			local street = GetStreetNameFromHashKey(GetStreetNameAtCoord(coords.x, coords.y, coords.z))
			TriggerServerEvent('Stinky_selldrugs:triggerDispatch', coords, street, false)
			ESX.ShowNotification('~b~Osoba nie jest zainteresowana')
...
RegisterNetEvent('Stinky_selldrugs:showNotify')
AddEventHandler('Stinky_selldrugs:showNotify', function(pos, street, msg)
