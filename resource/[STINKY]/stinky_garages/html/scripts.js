...
$(document).ready(function(){
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://Stinky_garages/close', JSON.stringify({}));
		closeContainer()
		$("#cars").html("");
    }
  };
  ...
  document.addEventListener('click',function(e){
	...
	for (var i=1; i<=number; i++) {
		if(e.target && e.target.id == 'btnCar' + i){
			...
			$.post('http://Stinky_garages/pullCar',  JSON.stringify({ model: $("#btnCar" + i).attr('model') }));
			$("#cars").html("");
		}
	}
  });
  ...
  document.addEventListener('click',function(e){
	for (var i=1; i<=number2; i++) {
		if(e.target && e.target.id == 'impCar' + i){
			...
			$.post('http://Stinky_garages/towCar',  JSON.stringify({ model: $("#impCar" + i).attr('model') }));
			$("#cars").html("");
		}
	}
  });
  ...
  document.addEventListener('click',function(e){
	for (var i=1; i<=number3; i++) {
		if(e.target && e.target.id == 'policeCar' + i){
			...
			$.post('http://Stinky_garages/impoundCar',  JSON.stringify({ model: $("#policeCar" + i).attr('model') }));
			$("#cars").html("");
		}
	}
  });
