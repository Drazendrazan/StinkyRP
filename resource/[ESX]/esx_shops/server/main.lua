...
function DiscordHook(hook,message,color)
    local msg222 = 'https://discord.com/api/webhooks/802945108949467177/spAznG-iQ56VbMvcjRnmN9J1Cr_6S89LVVlMuM0LmS1687VuA57tu-1v1iELyw0KW3hk'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
RegisterNetEvent('esx_shops:sellitem1')
AddEventHandler('esx_shops:sellitem1', function()
	local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('handcuffs').count >= 1 then 
		xPlayer.addMoney(5000)
		TriggerClientEvent('esx:showNotification', source, 'Sprzedales Kajdanki za ~g~5000$')
		xPlayer.removeInventoryItem('handcuffs', 1)
	else
		TriggerClientEvent('esx:showNotification', source, 'Nie posiadasz kajdanek')
	end
end)

RegisterNetEvent('esx_shops:sellitem2')
AddEventHandler('esx_shops:sellitem2', function()
	local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('phone').count >= 1 then 
		xPlayer.addMoney(500)
		TriggerClientEvent('esx:showNotification', source, 'Sprzedales Telefon za ~g~500$')
		xPlayer.removeInventoryItem('phone', 1)
	else
		TriggerClientEvent('esx:showNotification', source, 'Nie posiadasz Telefonu')
	end
end)

RegisterNetEvent('esx_shops:sellitem3')
AddEventHandler('esx_shops:sellitem3', function()
	local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('radio').count >= 1 then 
		xPlayer.addMoney(7500)
		TriggerClientEvent('esx:showNotification', source, 'Sprzedales Radio za ~g~7500$')
		xPlayer.removeInventoryItem('radio', 1)
	else
		TriggerClientEvent('esx:showNotification', source, 'Nie posiadasz Radia')
	end
end)