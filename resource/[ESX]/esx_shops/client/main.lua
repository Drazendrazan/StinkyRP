...
function OpenShopMenu(zone)
	...
	if zone == 'Lombard' then
		table.insert(elements, {label = 'Sprzedaj Przedmioty', value = 'sprzedaj'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		elseif data.current.value == 'sprzedaj' then
			menu.close()
			Sprzedaj()
		else
...
function Sprzedaj()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'waterkoxcignijmi',
        {
            ...
            elements = {
				{label = 'Telefon <span style="color: #7cfc00;">$250</span>', value = '1'},
				{label = 'Kajdanki <span style="color: #7cfc00;">$5000</span>', value = '2'},
				{label = 'Radio <span style="color: #7cfc00;">$7500</span>', value = '3'},
            }
        },
        function(data, menu)
          	if data.current.value == '1' then
				TriggerServerEvent('esx_shops:sellitem2')
			elseif data.current.value == '2' then
				TriggerServerEvent('esx_shops:sellitem1')
			elseif data.current.value == '3' then
				TriggerServerEvent('esx_shops:sellitem3')
			end
        end,
        function(data, menu)
             menu.close()
