...
AddEventHandler('esx_jb_outlawalert:notifyShots', function(coords, text, isPolice)
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		...
		TriggerEvent('chat:addMessage', {
			template = '<div class="chat-message" style="padding: 5px 10px 5px; margin: 7px; background-color: rgba(10, 10, 10, 0.55); border-radius: 4px;"><i class="fas fa-exclamation-circle"style="font-size:13px;color:rgb(255,255,255,0.7)"></i>&ensp;<font color="FFFFFF" style="font-weight: bold;">[{0}]: </font><font color="white">{1}</font></div>',
			args = { '^0^3Centrala^0', text }
		})
    	Citizen.CreateThread(function()
        	local alpha = 250 
        	while true do 
