...
RegisterCommand("jail", function(src, args, raw)
	...
	--if xPlayer.group == 'admin' or xPlayer.group == 'superadmin' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.job.name == 'police' then
		--[[if args[1] and GetPlayerName(args[1]) ~= nil and tonumber(args[2]) then
			TriggerEvent('esx_wykuriwajpedale:gigaczadowytrigger', tonumber(args[1]), tonumber(args[2] * 60))
		else
...
RegisterServerEvent('esx_wykuriwajpedale:gigaczadowytrigger')
AddEventHandler('esx_wykuriwajpedale:gigaczadowytrigger', function(target, jailTime)
	...
	MySQL.Async.fetchAll('SELECT * FROM jail WHERE identifier=@id', {['@id'] = identifier}, function(result)
		...
		for i=1, #xPlayers, 1 do
			  ...
			  print('esx_jailer: JAK COS TO MOZE WYWALAC!')
