...
AddEventHandler('odznaka', function()
	...
	local message = 'Pokazuje odznakę SASP: '  .. imie .. ' - [' .. badge .. '] ' .. stopien ..''
	local czy_wazna
	...
	TriggerClientEvent('esx:dowod_mariuszeksasp', -1,_source, '~w~'..name.firstname..' '..name.lastname,'Stopień: ~b~'..stopien, 'Numer odznaki: ~o~' .. badge .. '~w~~s~\nOdznaka jest ważna: '..czy_wazna)
...
AddEventHandler('emsodznaka', function()
	...
	local job = xPlayer.job
	local stopien = job.grade_label
	local message = 'Pokazuje Legitymację SAMS: ' .. imie .. ' - '.. stopien
	local czy_wazna
	...
	TriggerClientEvent('esx:dowod_mariuszeksams', -1,_source, ''..name.firstname..' '..name.lastname,'Legitymacja ~p~SAMS','~w~Stopień: ~b~'..job.grade_label..'~n~~s~Legitymacja jest ważna: '..czy_wazna)
	TriggerClientEvent('esx_dowod:sendProximityMessage', -1, _source, _source, message)
end)
...
AddEventHandler('psychologodznaka', function()
	...
	local job = xPlayer.job
	local stopien = job.grade_label
	local message = 'Pokazuje Legitymację Psychologa: ' .. imie .. ' - '.. stopien
	local czy_wazna
...
AddEventHandler('dojodznaka', function()
	...
	local job = xPlayer.job
	local stopien = job.grade_label
	local message = 'Pokazuje Legitymację DOJ: ' .. imie .. ' - '.. stopien
	local czy_wazna
...
AddEventHandler('lscodznaka', function()
	...
	local job = xPlayer.job
	local stopien = job.grade_label
	local message = 'Pokazuje Plakietkę LST: ' .. imie .. ' - '.. stopien
	local czy_wazna
	...
	TriggerClientEvent('esx:dowod_mariuszeklsc', -1,_source, ''..name.firstname..' '..name.lastname,'Plakietka ~o~LST','Stopień: ~b~'..job.grade_label..'~n~~s~Plakietka jest ważna: '..czy_wazna)
	TriggerClientEvent('esx_dowod:sendProximityMessage', -1, _source, _source, message)
end)