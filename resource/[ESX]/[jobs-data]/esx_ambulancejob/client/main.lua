...
AddEventHandler('esx_ambulancejob:sygnal', function()
	Citizen.Wait(100)
	SendDistressSignalA()
	local ems = exports['esx_scoreboard']:BierFrakcje('ambulance')
	if ems < 2 then
		SendDistressSignalP()
	end
end)
...
function SendDistressSignalA()
	exports['gcphone']:hasPhone(function(hasPhone)
		if hasPhone then
			...
			TriggerEvent('Stinky-alert:callNumberD', data)
		else
...
function RemoveItemsAfterRPDeath()
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()			
			if choosedHospital == 'ls' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlaceLS)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlaceLS)
				RespawnPed(PlayerPedId(), Config.RespawnPlaceLS)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Pillbox", 'teleportszpital')
			elseif choosedHospital == 'sandy' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlaceSANDY)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlaceSANDY)
				RespawnPed(PlayerPedId(), Config.RespawnPlaceSANDY)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Sandy Shores", 'teleportszpital')
			elseif choosedHospital == 'paleto' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlacePALETO)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlacePALETO)
				RespawnPed(PlayerPedId(), Config.RespawnPlacePALETO)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Paleto Bay", 'teleportszpital')
			elseif choosedHospital == 'bs1' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlacebs1)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlacebs1)
				RespawnPed(PlayerPedId(), Config.RespawnPlacebs1)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Paleto Bay", 'teleportszpital')
			elseif choosedHospital == 'bs2' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlacebs2)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlacebs2)
				RespawnPed(PlayerPedId(), Config.RespawnPlacebs2)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Paleto Bay", 'teleportszpital')
			elseif choosedHospital == 'bs3' then
				ESX.SetPlayerData('lastPosition', Config.RespawnPlacebs3)
				TriggerServerEvent('esx:updateLastPosition', Config.RespawnPlacebs3)
				RespawnPed(PlayerPedId(), Config.RespawnPlacebs3)
				TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
				StopScreenEffect('DeathFailOut')
				DoScreenFadeIn(800)
				TriggerServerEvent('exile_logs:triggerLog', "Przeteleportował się na szpital Paleto Bay", 'teleportszpital')
				...
				if GetGameTimer() - pressStart > 3000 then
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'rp_dead', {
						...
						elements = {
							...
							{ label = ('-------------'), value = '' },
							{ label = ('Bezpieczna Strefa (Zlomowisko)'), value = 'bs1' },
							{ label = ('Bezpieczna Strefa (Autostrada)'), value = 'bs2' },
							{ label = ('Bezpieczna Strefa (PetShop)'),  value = 'bs3'},
						}
					}, function (data, menu)
						...
						elseif data.current.value == 'paleto' then
							...
							RemoveItemsAfterRPDeath()
						elseif data.current.value == 'bs1' then
							choosedHospital = 'bs1'
							RemoveItemsAfterRPDeath()
						elseif data.current.value == 'bs2' then
							choosedHospital = 'bs2'
							RemoveItemsAfterRPDeath()
						elseif data.current.value == 'bs3' then
							choosedHospital = 'bs3'
							RemoveItemsAfterRPDeath()
						end
						menu.close()
...
RegisterNetEvent('kill:effect')
AddEventHandler('kill:effect', function()
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									local lib, anim = 'mini@cpr@char_a@cpr_str', 'cpr_pumpchest'
										...
										ESX.Streaming.RequestAnimDict(lib, function()
											TaskPlayAnim(PlayerPedId(), lib, anim, 8.0, 8.0, 10000, 1, 0, false, false, false)
											--TaskPlayAnim(PlayerPedId(), lib, anim, 8.0, -8.0, 10000, 0, 0, false, false, false)
										end)
										exports["stinky_taskbar"]:taskBar(10000, "Trwa wskrzeszanie", false, true)

									TriggerServerEvent('esx_ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('esx_ambulancejob:reviveniggers', GetPlayerServerId(closestPlayer))
									IsBusy = false
						...
						if IsPlayerDead(closestPlayer) then
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions_test', {
								...
								elseif ac == 'death' then
									...
									exports["stinky_taskbar"]:taskBar(5000, "Badanie pacjenta", false, true)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'alluniforms' then
			...
			if PlayerData.job.grade >= 0 then
				table.insert(elements2, {label = 'Nurse', value = 'pielegniarz_wear'})
			  end
			  if PlayerData.job.grade >= 1 then
				table.insert(elements2, {label = 'Paramedic', value = 'ratownik_wear'})
			  end
			  if PlayerData.job.grade >= 2 then
				table.insert(elements2, {label = 'Senior Paramedic', value = 'stratownik_wear'})
			  end
			  if PlayerData.job.grade >= 3 then
				table.insert(elements2, {label = 'Doctor', value = 'lekarz_wear'})
			  end
			  if PlayerData.job.grade >= 4 then
				table.insert(elements2, {label = 'Senior Doctor', value = 'lekarzsoru_wear'})
			  end
			  if PlayerData.job.grade >= 5 then
				table.insert(elements2, {label = 'Medical Specialist', value = 'lekarzspecjalista_wear'})
			  end
			  if PlayerData.job.grade >= 6 then
				table.insert(elements2, {label = 'Assistant Neurosurgeon', value = 'doktor_wear'})
			  end
			  if PlayerData.job.grade >= 7 then
				table.insert(elements2, {label = 'Surgeon', value = 'chirurg_wear'})
			  end
			  if PlayerData.job.grade >= 8 then
				table.insert(elements2, {label = 'Neurosurgeon', value = 'neurochirurg_wear'})
			  end
...
function OpenVehicleSpawnerMenu(partNum)
	...
	for i, group in ipairs(Config.VehicleGroups) do
		...
		if (PlayerData.job.name == 'ambulance' and PlayerData.job.grade >= 12) then
			if #elements2 > 0 then
...
RegisterNetEvent('esx_ambulancejob:reviveniggers')
AddEventHandler('esx_ambulancejob:reviveniggers', function(notBlock)
	...
	--[[if IsDead and not notBlock then
		...
		blockShooting = GetGameTimer() + (10 * 20000)
	end--]]
	TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
