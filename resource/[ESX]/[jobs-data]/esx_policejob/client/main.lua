...
CreateThread(function()
	...
	while true do
		...
		if armour > 75 then
			TriggerServerEvent('move_logs:triggerLog', 'Posiada kamze powyzej 50%', 10181046, 'https://discord.com/api/webhooks/930492684869730316/nrnXT_neb9PEcknBFZeWpOce6ZnqY4lD2wDvVPOmQknZw1NK5Fo602Vq5VwOctulN3nN')
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = ('Police Uniforms'), value = 'job_wear' }		
	}
  
	if PlayerData.job.name == 'police' then
		table.insert(elements, {label = 'Private', value = 'player_dressing' })
		if PlayerData.hiddenjob.name == 'police' then
		...
		if closestPlayer ~= -1 and closestDistance <= 3.0 then
			...
			if action == 'identity_card' then
				if IsPedCuffed(targetPed) then
					exports["stinky_taskbar"]:taskBar(1000, "Szukanie dokumentow", false, true)					
					TriggerServerEvent('dowod', GetPlayerServerId(closestPlayer))
				end
			elseif action == 'agresivehandcuff' then
				...
				if not IsPedCuffed(targetPed) then
					...
					if distance <= 2.0 then
						...
						exports["stinky_taskbar"]:taskBar(4000, "Zakuwanie", false, true)
					...
					if distance <= 2.0 then
						...
						exports["stinky_taskbar"]:taskBar(6000, "Odkuwanie", false, true)
			...
			--	if not exports['esx_property']:isProperty() then
					if not IsPedCuffed(closestPlayer) then
					...
					exports["stinky_taskbar"]:taskBar(1000, "Zakuwanie/Odkuwanie", false, true)
			...
			elseif action == 'body_search' then
				if IsPedCuffed(targetPed) then
					if IsPedCuffed(targetPed) or hasAnim1 or hasAnim2 and not IsPlayerDead(closestPlayer) then
						...
						exports["stinky_taskbar"]:taskBar(2000, "Przeszukiwanie", false, true)
			...
			elseif action == 'drag' then
				if not Dragging then
					if IsPedCuffed(targetPed) then
						exports["stinky_taskbar"]:taskBar(1000, "Łapanie/Puszczanie", false, true)
			...
			elseif action == 'put_in_vehicle' then
				if IsPedCuffed(targetPed) then
					exports["stinky_taskbar"]:taskBar(1000, "Wkładzanie do pojazdu", false, true)
					if Dragging then
						TriggerServerEvent('esx_policejob:drag', Dragging)
					end
			...
			elseif action == 'out_the_vehicle' then
				if IsPedCuffed(targetPed) then
					exports["stinky_taskbar"]:taskBar(1000, "Wyciąganie z pojazdu", false, true)
					TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
				end
			elseif action == 'bagol1' then
				if IsPedCuffed(targetPed) then
					exports["stinky_taskbar"]:taskBar(1000, "Wkładzanie do bagażnika", false, true)
					if Dragging then
						TriggerServerEvent('esx_policejob:drag', Dragging)
					end
								
					TriggerServerEvent('space:putTargetInTrunk', GetPlayerServerId(closestPlayer))
				end
			elseif action == 'bagol2' then
				if IsPedCuffed(targetPed) then
					exports["stinky_taskbar"]:taskBar(1000, "Wyciąganie z bagażnika", false, true)
					TriggerServerEvent('space:outTargetFromTrunk', GetPlayerServerId(closestPlayer))
				end
			...
			elseif action == 'gsr' then
				...
				exports["stinky_taskbar"]:taskBar(5000, "Sprawdzanie prochu", false, true)
			...
			elseif action == 'nadajnik' then
				if IsPedCuffed(targetPed) then
					ESX.TriggerServerCallback('space_gps:checkHasGPS', function(cb)
						if cb.has == true then
							exports["stinky_taskbar"]:taskBar(6000, "Zdejmowanie nadajnika", false, true)
							ESX.ShowNotification("Ściągasz ~r~nadajnik GPS~w~ dla ~y~" .. cb.firstname .. " " .. cb.lastname)
							TriggerServerEvent('space_gps:opaska', false, cb)
						else
							exports["stinky_taskbar"]:taskBar(6000, "Zakładanie nadajnika", false, true)
							ESX.ShowNotification("Zakładasz ~r~nadajnik GPS~w~ dla ~y~" .. cb.firstname .. " " .. cb.lastname)
							TriggerServerEvent('space_gps:opaska', true, cb)
						end
...
--	TriggerEvent('esx_policejob:handcuff')
	...
	CreateThread(function()
		if IsHandcuffed then
			local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
			if not exports['space_trunk']:checkInTrunk() then
			...
			if exports['space_trunk']:checkInTrunk() then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elseif data.current.value == 'lorneta' then
			TriggerEvent('space_lorneta:lornetaon')
					...
					elseif DoesEntityExist(vehicle) then
						...
						elseif action == 'fix_vehicle' then
							if(not IsPedInAnyVehicle(playerPed)) then
								...
								TriggerServerEvent('space:pay', 500)
							end
				...
				ESX.TriggerServerCallback('esx_policejob:checkSearch2', function(cb)
					if cb == true then
						...
						if #(playerCoords - targetCoords) <= 3.0 then
							...
							exports["stinky_taskbar"]:taskBar(1000, "Zabieranie przedmiotu", false, true)
							OpenBodySearchMenu(target)
						end
					else
						ESX.ShowNotification("Nice try bitch :)")
					end
				end, serverId)
			end
		end, function(data, menu)
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				exports["stinky_taskbar"]:taskBar(5000, "Sparawdzanie tablic", false, true)	
				if found then
...
AddEventHandler('esx_policejob:putInTrunk', function(cop)
	if IsHandcuffed then				
		TriggerEvent('space:forceInTrunk', cop)
	end
end)
...
AddEventHandler('esx_policejob:OutTrunk', function(cop)
	if IsHandcuffed then
		...
		TriggerEvent('space:forceOutTrunk', cop)
		...
		if vehicle and vehicle ~= 0 then
			...
			if maxSeats >= 0 then
				...
				if freeSeat ~= nil then					
					...
					TriggerEvent('space_blackout:belt', true)
				end
			end
		end
	end
...
CreateThread(function()
	while true do
		...
		if IsHandcuffed then
            ...
            if not IsEntityPlayingAnim(playerPed, 'mp_arresting', 'idle', 3) and not exports['space_trunk']:checkInTrunk() then
...
function MenuBroni()
	ESX.UI.Menu.CloseAll()
	local elements = {}
	table.insert(elements, {label = 'Pałka Policyjna', value = 'nightstick'})
	table.insert(elements, {label = 'Paralizator', value = 'stungun'})
	table.insert(elements, {label = 'Vintage pistol', value = 'vintagepistol'})
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
	{
		...
		if weapon ~= 'radio' and weapon ~= 'ggps' and weapon ~= 'gpsill' and weapon ~= 'leki' and weapon ~= 'bandage' and weapon ~= 'grip' and weapon ~= 'mustwinwiekszy' and weapon ~= 'mustwin' and weapon ~= 'mustwinmaly' and weapon ~= 'flashlight' and weapon ~= 'nightstick' and weapon ~= 'stungun' and weapon ~= 'pistol' and weapon ~= 'snspistol' and weapon ~= 'combatpistol' and weapon ~= 'heavypistol' and weapon ~= 'pistol_ammo' and weapon ~= 'panicbutton' and weapon ~= 'vintagepistol' and weapon ~= 'snspistol_mk2' and weapon ~= 'phone' and weapon ~= 'redbull' and weapon ~= 'bread' and weapon ~= 'water' and weapon ~= 'ceramicpistol' and weapon ~= 'pistol_mk2' then
			LSPDNotification('SASP', '~y~Pobrano bron z zbrojowni', '~b~Bron~s~: ~r~' .. ESX.GetWeaponLabel(weapon), 'CHAR_CALL911', 1)
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'radio' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Radio', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'ggps' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~GPS', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'panicbutton' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Panic Button', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'gpsill' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~GPS Śledzący', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'leki' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Tabletki', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'bandage' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Bandaż', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'grip' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Uchwyt', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'mustwinwiekszy' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Wkład Balistyczny', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'mustwin' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Wkład Balistyczny', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'mustwinmaly' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano przedmiot z zbrojowni', 'Przedmiot~s~: ~b~Wkład Balistyczny', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'flashlight' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Latarka', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'nightstick' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pałka Policyjna', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'stungun' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Paralizator', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'pistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pistolet', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'snspistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pukawka', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'combatpistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pistolet Bojowy', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'pistol_mk2' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pistolet Bojowy', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'ceramicpistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Pistolet Bojowy', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'heavypistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Ciężki Pistolet', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'vintagepistol' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Ciężki Pistolet', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		elseif weapon == 'snspistol_mk2' then
			TriggerEvent('FeedM:showAdvancedNotification', 'San Andreas State Police', '~y~Pobrano broń z zbrojowni', 'Broń~s~: ~b~Ciężki Pistolet', 'lssd')
			TriggerServerEvent('esx_policejob:giveItem', data.current.value, 1)
		end
	end, function(data, menu)
...
OpenSERTArmoryMenu = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sert_armory', {
		...
		if data.current.value == 'put_weapon' then
			TriggerEvent('space:putInventoryItem', 'society_sert')
		elseif data.current.value == 'get_weapon' then
			TriggerEvent('space:getInventoryItem', 'society_sert')
		end
	end, function(data, menu)
		menu.close()
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		if data.current.value == 'put_stock' then
			TriggerEvent('esx_policejob:putInventoryItem', 'society_police')
		elseif data.current.value == 'put_weapon' then
			TriggerEvent('esx_policejob:putInventoryItem', 'society_sert')
		elseif data.current.value == 'get_wypo' then
			OpenGetWypoMenu()
		elseif data.current.value == 'get_weapon' then
			MenuBroni()
		elseif data.current.value == 'get_stock' then
			TriggerEvent('esx_policejob:getInventoryItem', 'society_police')
		end
	end, function(data, menu)
		menu.close()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			if not IsThisModelAHeli(model) and not IsThisModelAPlane(model) and not IsThisModelABoat(model) then
				...
				CreateThread(function()
					exports["stinky_taskbar"]:taskBar(5000, "Trwa Odblokowywanie", false, true)
...
RegisterNetEvent('space_lorneta:lornetaon') --Just added the event to activate the binoculars
AddEventHandler('space_lorneta:lornetaon', function()
	helicam = not helicam
	ESX.UI.Menu.CloseAll()
end)
        ...
        elseif data.current.value == 'handcuff' then
            if not exports['esx_property']:isInProperty() then
				--if CanCuff(closestPlayer, true) then
					exports["stinky_taskbar"]:taskBar(2000, "Skuwanie / Rozkuwanie", false, true)
					TriggerServerEvent('esx_policejob:handcuffhype', GetPlayerServerId(closestPlayer))
					TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer), '~o~Zostałeś/aś zakuty przez ~g~[' ..GetPlayerServerId(PlayerId())..']')
					ESX.ShowNotification('~o~Zakułeś/Odkułeś ~g~[' .. GetPlayerServerId(closestPlayer) ..']')
				--else
