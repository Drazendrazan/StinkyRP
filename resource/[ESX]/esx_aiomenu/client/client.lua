...
AddEventHandler('esx_exilemenu:getUserInfo', function(firstname, lastname, kursy)
	Siemanko.firstname = firstname
	Siemanko.lastname = lastname
  Siemanko.kursy = kursy
end)
...
function openGeneral(reopen)
  ...
  if PlayerData.job and PlayerData.job.name == 'police' and PlayerData.job.name == 'offpolice' and PlayerData.job.name == 'ambulance' and PlayerData.job.name == 'offambulance' and PlayerData.job.name == 'mecano' and PlayerData.job.name == 'offmecano' then
    SendNUIMessage({
      type = 'updateInfo',
...
RegisterCommand('dowod', function(...)
  if GetGameTimer() > lastGameTimerId then
    TriggerServerEvent('dowod')
		...
		TriggerEvent("chatMessage", "^1[SYSTEM]> ", {255, 255, 0}, "Nie możesz tak często wyciągać dowodu!")
	end
end)
...
RegisterNUICallback('psychologodznaka', function(...)
  if PlayerData.job.name == 'psycholog' then
    if GetGameTimer() > lastGameTimerpsychol then
      ...
      TriggerEvent("chatMessage", "^1[SYSTEM]> ", {255, 255, 0}, "Nie możesz tak często wyciągać legitymacji!")
      SetNuiFocus(false,false)
    end
  end
end)
...
RegisterNUICallback('show_bind', function(data)
  TriggerEvent('Stinky:showkeybinding', true)
end)

RegisterNUICallback('show_hud', function(data)
...
RegisterNUICallback('show_hud1', function(data)
  ExecuteCommand('eq')
end)
...
RegisterCommand('wizytowka', function(...)
  if GetGameTimer() > lastGameTimerPhone then
	  TriggerServerEvent('wizytowka')
		...
		TriggerEvent("chatMessage", "^1[SYSTEM]> ", {255, 255, 0}, "Nie możesz tak często wyciągać wizytówki!")
  end
end)
...
RegisterNUICallback('NUIopenAmbulance', function()
  if PlayerData.job.name == 'ambulance' then
    ...
    TriggerServerEvent('Stinky:detectaio')
  end
end)
