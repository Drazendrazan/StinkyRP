...
local lickaB = getlicenseB(xd)
local lickaA = getlicenseA(xd)
local lickaC = getlicenseC(xd)
local lickaW = getlicenseW(xd)
local lickaZdrowie = getlicenseZdrowie(xd)
local lickaOC = getlicenseOC(xd)
...
TriggerClientEvent('ExileRP:Dow', -1, xd, name.firstname..' '..name.lastname, '~y~' .. name.sex .. ', ' ..name.dateofbirth .. ', ' .. name.height .. 'cm', str )
end)
...
RegisterServerEvent('SpaceRP:CarDealer')
AddEventHandler('SpaceRP:CarDealer', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)	
	local name = GetDowodKurwa(xPlayer.identifier)
		if name.phone_number == nil then 
	...
	TriggerClientEvent('SpaceRP:CarDealer', -1,_source, name.firstname.. ' ' ..name.lastname, '~y~'..xPlayer.job.label .. ' - ' .. xPlayer.job.grade_label, '~o~Numer kontaktowy: ~s~' .. name.phone_number)
end)
...
RegisterServerEvent('ExileRP:Badges')
AddEventHandler('ExileRP:Badges', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local name = GetDowodKurwa(xPlayer.identifier)
	...
	if name.odznaka ~= 0 then 
		badgeFull = '['..name.odznaka..']'
	end

	local message = '^6Pokazuje odznakę policyjną: '  .. imie .. ' - [' .. badgeFull .. '] ' .. stopien ..''

	TriggerClientEvent('ExileRP:Badge', -1,_source, '~w~'..name.firstname..' '..name.lastname, '~y~Legitymacja - SASP', '~w~Stopień: ~y~'..stopien .. '\n~w~Numer odznaki: ~y~' .. badgeFull)
