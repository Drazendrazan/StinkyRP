...
local blacklist = {
	...
	'simpem',
	'simpy',
	'1̶̉͊3̷̐͒3̸̍͝7̴̉̂Ń̶͗Ḯ̶̏G̴̈̀G̴̊̕Ẽ̶̔R̴̽͋S̸̋͛꧁༒☬ᤂℌ໔ℜ',
	'3̸̍͝',
	'Ń̶͗',
	'Ḯ̶̏',
	'G̴̊̕',
...
function getIdentity(source)
  ...
  AddEventHandler('chatMessage', function(source, name, message)
	  if string.sub(message, 1, string.len("/")) ~= "/" then
		...
		if xPlayer ~= nil then
			TriggerClientEvent("sendProximityMessage", -1, source, xPlayer.name, xPlayer.group, message)
			--exports['AdrenalinaRP']:SendLog(message, source, 'https://discord.com/api/webhooks/849704672906313728/mIaE_2RfddWBS4qDk7hZQxbf44Mb7kqUFRy9KLpx3_oajuR_SXFgEsgiNZDsbb-Lfjj0', 1752220)
		end
      end
      CancelEvent()
  end)
...
RegisterServerEvent('ogloszenie:bot')
AddEventHandler('ogloszenie:bot', function(id,message)
	TriggerClientEvent('esx_rpchat:sendProximityMessageTask', -1, id, message, { 36, 173, 228 })
end)
...
RegisterCommand('reportoff', function(source, args, rawCommand)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == 'best' then
        TriggerClientEvent('changeReportStatus', source)
	else
		TriggerClientEvent('esx:showNotification', source, '~r~Reporta moze wyłaczyc tylko ranga ~b~Zarzad')
    end
  end)
  ...
  AddEventHandler('chatMessage', function(source, color, msg)
	  ...
	  if cm[1] == "/o" then
		  ...
		  if tablelength(cm) > 1 then
			  ...
			  if Groups[ESX.GetPlayerFromId(source).getGroup()] then
			  TriggerClientEvent("sendadmin", -1, source, names1, textmsg)
			else
...
RegisterCommand('twt', function(source, args, raw)
	...
	if args[1] ~= nil then
		for k,v in ipairs(GetPlayerIdentifiers(source))do
			local muteData = isMuted(v)
			if muteData ~= false then
				TriggerClientEvent("esx:showNotification", xPlayer.source, 'Jesteś wyciszony!', 5000, 'primary')
				break
			else
				for k, v in pairs(blacklist) do
					if string.match(string.lower(msg), v) then
						DropPlayer(source, 'Zostal zkickowany za napisanie zblacklistowanego slowa na twt')
						TriggerClientEvent('esx:showNotification', _source , 'Zostal zkickowany za napisanie zblacklistowanego slowa na dw')
						return
					end
				end
				TriggerClientEvent("pokaTwTc", xPlayer.source, name.firstname .. " ".. name.lastname,  msg)
				break
			end
		end
	end
...
RegisterServerEvent('sendProximityMessageTwitterServer')
AddEventHandler('sendProximityMessageTwitterServer', function(name, message)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAccountMoney('bank', 250)
	TriggerClientEvent('esx:showNotification', source, '~r~Zabrano ~w~250 ~g~$ ~w~Dolarów ~r~z konta bankowego')
	TriggerClientEvent('pokaTwT', -1, xPlayer.source, name, message)
end)
...
AddEventHandler('sendProximityMessageDwServer', function(message)
	...
	TriggerClientEvent('pokaDw', -1, xPlayer.source, xPlayer.hiddenjob.label, message)
end)


ESX.RegisterCommand('twtmute', {'superadmin', 'best'}, function(xPlayer, args, showError)
	...
	if time ~= nil then
		...
		TriggerClientEvent("esx:showNotification", xPlayer.source, 'Pomyślnie wyciszono na: ' .. time .. 's', 5000, 'primary')
...
ESX.RegisterCommand('unmute', {'superadmin', 'best'}, function(xPlayer, args, showError)
	...
	TriggerClientEvent("esx:showNotification", xPlayer.source, 'Pomyślnie odciszono!', 5000, 'primary')
end, true, {help = 'unmute', validate = true, arguments = {
	{name = 'playerId', help = 'Id gracza', type = 'number'},
}})

RegisterCommand('sasp', function(source, args)
	local _source = source
	if args[1] ~= nil then
		local name = getIdentity(source)
		local xPlayer = ESX.GetPlayerFromId(source)
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx:showNotification', source, '~r~Nie jestes w SASP!')
		end
	end
end)
...
RegisterCommand('news', function(source, args)
	...
	if args[1] ~= nil then
		...
		local xPlayer = ESX.GetPlayerFromId(source)
                local amount = 100
		if xPlayer.job.name == 'police' or xPlayer.job.name == 'kawiarz' or xPlayer.job.name == 'krawiec' or xPlayer.job.name == 'ambulance' or xPlayer.job.name == 'mecano' or xPlayer.job.name == 'cardealer' or xPlayer.job.name == 'kurier' then
			msg = Emojit(table.concat(args, " "))
			if CheckPhone(source) <= 0 then
			 TriggerClientEvent('esx:showNotification', source, '~r~Nie posiadasz telefonu.')
                        return
...
RegisterCommand("dwoff", function(source, args)
	TriggerClientEvent('neey:dwStatus', source, false)
end)

RegisterCommand("dwon", function(source, args)
	TriggerClientEvent('neey:dwStatus', source, true)
end)
...
RegisterCommand('dw', function(source, args)
	...
	if xPlayer.job.name == 'police' or xPlayer.job.name == 'ambulance' or xPlayer.job.name == 'offambulance' or xPlayer.job.name == 'offpolice' then
		TriggerClientEvent('esx:showNotification', source, '~r~Nie możesz używać tego chatu!')
		return
	else
		if args[1] == nil then
			...
			if (xPlayer.job.name == "police" or xPlayer.job.name == "offpolice" or xPlayer.job.name == "ambulance" or xPlayer.job.name == "offambulance") then
				...
				if CheckPhone(source) <= 0 then
					...
					for k, v in pairs(blacklist) do
						if string.match(string.lower(msg), v) then
							TriggerClientEvent('esx:showNotification', source, '~r~Zostal zkickowany za napisanie zblacklistowanego slowa na dw.')
							DropPlayer(source, 'Zostal zkickowany za napisanie zblacklistowanego slowa na dw')
							return
						end
					end
					TriggerClientEvent("pokaDwc",source, msg)
				end
			end
	end
end)
