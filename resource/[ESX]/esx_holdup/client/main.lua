...
AddEventHandler('esx_holdup:setblip', function(position)
	...
	TriggerEvent('chat:addMessage', {
		...
		args = { '^0^3Centrala^0', 'Trwa', _U('shop_robbery') }
	})
end)
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 10.0)then
				...
				if not holdingup then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2) then
						ESX.ShowFloatingHelpNotification('~b~ NAPAD NA ~s~[~b~'..v.nameofstore..'~s~]', vec3(v.position.x, v.position.y, v.position.z + 0.45))
					end
