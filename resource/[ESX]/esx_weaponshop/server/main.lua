...
AddEventHandler('weaponshop:buypistol', function()
	...
	if xPlayer.getInventoryItem('pistol').count <= 1 then
		...
		TriggerClientEvent('esx:showNotification', source, 'Masz max pistoli przy sobie!')
	end
end)
...
AddEventHandler('weaponshop:buynoz', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('knife').count <= 1 then
		...
		TriggerClientEvent('esx:showNotification', source, 'Masz max nozy przy sobie!')
	end
end)
...
AddEventHandler('weaponshop:buymagazynek', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('pistol_ammo').count <= 250 then
		...
		TriggerClientEvent('esx:showNotification', source, 'Masz max ammo przy sobie!')
	end
end)
...
AddEventHandler('esx_weashop:buyItem', function(itemName, price, zone)
	...
	if zone=="BlackWeashop" then
		if account.money >= price then
			...
			DiscordHook3('StinkyRP.pl', wiadomosc, 11750815)
		else
