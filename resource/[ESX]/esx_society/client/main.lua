...
function RefreshBussHUD()
	...
	if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'urzednik6' or PlayerData.job.grade_name == 'zcadyrektora' or PlayerData.job.grade_name == 'zcakomenda' or PlayerData.job.grade_name == 'uber' or PlayerData.job.grade_name == 'kierownik' or  PlayerData.job.grade_name == 'kapitan3' then
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	if PlayerData.job and PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'urzednik6' or PlayerData.job.grade_name == 'zcadyrektora' or PlayerData.job.grade_name == 'zcakomenda' or PlayerData.job.grade_name == 'kierownik' or  PlayerData.job.grade_name == 'kapitan3' and 'society_' .. PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)
...
function OpenBossMenu2(society, close, options)
	...
	if options.showmoney then
		table.insert(elements, {label = 'Saldo <span style="color:gree#7cfc00;">'..fractionAccount..'$' , value = 'none'})
	end
...
function OpenManageEmployeesMenu(society)
    elements = {
        {label = _U('employee_list'), value = 'employee_list'},
        {label = _U('recruit'),       value = 'recruit'},
        --{label = "Zarządzaj licencjami", value = 'licenses'}
    }
...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		if (PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'police' or PlayerData.job.name == 'fire' ) then
			elements = {
				head = {_U('employee'), _U('grade'), 'Odznaka', 'Czas Służby [W Godzinach]', _U('actions')},
				rows = {}
			}
		else
			elements = {
		...
		if (PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'police' or PlayerData.job.name == 'fire') then
			for i=1, #employees, 1 do
				...
				--local ezz = (tostring(employees[i].job.czas))

				if employees[i].job.czas == 0 then
					ezz = 'Brak Aktywności'
				elseif employees[i].job.czas > 0 then
					ezz = ''.. employees[i].job.czas ..' Godziny'
				end
				...
				table.insert(elements.rows, {
					...
					cols = {
						...
						gradeBadge,
						ezz,
						'{{Odznaka |odznaka}} {{Zeruj Czas |zeruj_czas}} {{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}}'
					}
				})
			end
		else
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			if data.value == 'odznaka' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'odznaki', {
					...
					if amount == nil or amount < 1 then
						...
						menu2.close()
						TriggerServerEvent('esx_society:setBadge', employee.identifier, amount)
						Citizen.Wait(200)
			...
			elseif data.value == 'zeruj_czas' then
				ESX.TriggerServerCallback('esx_society:zeruj_czas', function()
					OpenEmployeeList(society)
				end, employee.identifier)
			end
		end, function(data, menu)
...
function OpenGiveLicenseMenu(employee, society)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give', {
		...
		menu.close()
		TriggerServerEvent('esx_society:giveLicense', employee.identifier, data.current.value)
		Citizen.Wait(500)
		OpenLicensesMenu(society)
	end, function(data, menu)
		menu.close()
...
function OpenGetLicenseMenu(employee, society)
	ESX.TriggerServerCallback('esx_society:getkurwaplayer', function(seu, sert, usms, dtu, heli, nurek, aiad)
		ESX.UI.Menu.CloseAll()
		elements = {}
		if seu then 
			table.insert(elements, {label = 'SEU', value = "seu"})
		elseif sert then
			table.insert(elements, {label = 'SERT', value = "sert"})
		elseif usms then
			table.insert(elements, {label = 'USMS', value = "usms"})
		elseif dtu then
			table.insert(elements, {label = 'DTU', value = "dtu"})
		elseif heli then
			table.insert(elements, {label = 'HELi', value = "heli"})
		elseif nurek then
			table.insert(elements, {label = 'NUREK', value = "nurek"})
		elseif aiad then
			table.insert(elements, {label = 'AIAD', value = "aiad"})
		end
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give', {
			...
			menu.close()
			TriggerServerEvent('esx_society:getLicense', employee.identifier, data.current.value)
			Citizen.Wait(500)
			OpenLicensesMenu(society)
		end, function(data, menu)
			menu.close()
...
function OpenLicensesMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployeeslic', function(employees)
		...
		local identifier = ''
			...
			elements = {
				head = {"Pracownik", "SEU", "SERT", "USMS", "DTU", "HELI", "NUREK", "AIAD", "Akcje"},
				rows = {}
			}
			...
			for i=1, #employees, 1 do
				...
				if employees[i].licensess.sert == true then
					licki[2] = '✔️'
				else
					licki[2] = "❌"
				end
				if employees[i].licensess.usms == true then
					licki[3] = '✔️'
				else
					licki[3] = "❌"
				end
				if employees[i].licensess.dtu == true then
					licki[4] = '✔️'
				else
					licki[4] = "❌"
				end
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			if data.value == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give', {
					...
					menu4.close()
					TriggerServerEvent('esx_society:giveLicense', employee.identifier, data4.current.value)
					Citizen.Wait(500)
					OpenLicensesMenu(society)
			...
			elseif data.value == 'take' then
									menu.close()
										...
										ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give', {
											...
											menu4.close()
											TriggerServerEvent('esx_society:getLicense', employee.identifier, data4.current.value)
											Citizen.Wait(500)
											OpenLicensesMenu(society)
										end, function(data4, menu4)
											menu4.close()
