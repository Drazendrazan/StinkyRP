...
AddEventHandler("definitelynotPK:LoadSlots", function()
	...
	Wait(2000)
	TriggerClientEvent("Stinky:updatebinds", _source, slots)
end)
...
CanUse = function(source, identifier, itemName, data)
    ...
    if (itemName == tostring('Brak') or itemName == nil) then
        ...
        if CheckWeapons(itemName) == true then
            ...
            if count > 0 then
				TriggerClientEvent('Stinky:IsWeapon', _source, itemName)
            else
            ...
            local count = xPlayer.getInventoryItem(itemName).count
            if count > 0 then
				TriggerClientEvent('Stinky:useItem', _source, to_use)
            else
                xPlayer.showNotification("Nie posiadasz tego przedmiotu! - Usunięto wpis z bazy danych")
                DeleteFromSlot(source, identifier, data)
            end
...
UpdateSlotsDB = function(sloty, item, identifier, source)
	local _source = source
	TriggerClientEvent("Stinky:updatebinds", _source, sloty)
...
RegisterServerEvent('Stinky:RegisterItemToSlot')
AddEventHandler('Stinky:RegisterItemToSlot', function(itemnum, item)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	RegisterItemToSlot(_source, itemnum, item)
end)

RegisterServerEvent('Stinky:UseItemFromBind')
AddEventHandler('Stinky:UseItemFromBind', function(num)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	UseSlot(_source, xPlayer.getIdentifier(), num)
end)

RegisterServerEvent('Stinky:deleteBind')
AddEventHandler('Stinky:deleteBind', function(num)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('Stinky:getBinds', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
