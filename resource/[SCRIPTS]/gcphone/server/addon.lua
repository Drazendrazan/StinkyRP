...
RegisterServerEvent('Stinky-alert:acceptedAlert')
AddEventHandler('Stinky-alert:acceptedAlert', function(dt, aO)
	...
	if actAlert.oneAccept == false then
		...
		TriggerClientEvent('Stinky-alert:accepted', aO)
	end
end)

RegisterServerEvent('Stinky-alert:startCall')
AddEventHandler('Stinky-alert:startCall', function(number, message, coords, player)
	local source = source
	local xPlayers = ESX.GetPlayers()
	...
	if number == 'police' then
		...
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			if xPlayer.job.name == 'police' then
				TriggerClientEvent('Stinky-alert:sendAlert', xPlayer.source, actAlert, player)
