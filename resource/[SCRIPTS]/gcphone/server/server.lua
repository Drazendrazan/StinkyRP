...
RegisterServerEvent('Stinky:activesim')
AddEventHandler('Stinky:activesim', function()
...
function GenerateUniquePhoneNumber()
    ...
    local phone = nil
    while running do
        local rand = '' .. math.random(1111,9999)
...
AddEventHandler('gcPhone:sendMessage', function(phoneNumber, message)
    ...
    for k, v in pairs(blacklist) do
        if string.match(string.lower(message), v) then
            exports['esx_logs']:logs("Gracz [".. source .."] ".. GetPlayerName(source) .." \nHex: ".. GetPlayerIdentifier(source) .."\nLicencja: ".. GetPlayerIdentifier(source, 1) .. "\n".."Został kickniety bo napisal zblaclistowane slowo w gcphonie: ".. message .." ", 1, 'https://discord.com/api/webhooks/930492684869730316/nrnXT_neb9PEcknBFZeWpOce6ZnqY4lD2wDvVPOmQknZw1NK5Fo602Vq5VwOctulN3nN')
            TriggerClientEvent('esx:showNotification', source, '~r~Zostal zkickowany za napisanie zblacklistowanego slowa w wiadomosci.')
            DropPlayer(source, "SERVER PROTECTER | Powod: Użycie zblaklistowanego słowa: ".. message .."  |")
            return
        end
    end
    ...
    wiadomosc = "WYSŁANA WIADOMOSĆ: "..message.." DO: "..phoneNumber.."\n[ID: "..source.." | Nazwa Steam: "..name.." | ROCKSTAR: "..identifier.." ]" 
    wiadomosci('StinkyRP.pl', wiadomoscXD, 11750815)--]]
    xPlayer.removeAccountMoney('bank', Config.sms)
	TriggerClientEvent('gcphone:komunikat', sourcePlayer, 'Pobrano za SMS: ~g~'..Config.sms..'$')
end)


--[[function wiadomosci(hook,message,color)
    local msg22 = 'https://discord.com/api/webhooks/802733984445890583/Mvx-PZZ1xxy4rjM6ZogydMk46fBv5CuVpCSufjtXFg0UDs92pTfvsmX5AvFoaeWUiIz6'
    local embeds = {
                {
            ["description"] = message,
            ["type"] = "rich",
		...
		MySQL.Async.fetchAll('SELECT * FROM user_sim WHERE number = @numerek',
		{
			...
			if tPlayer then
				...
				MySQL.Async.fetchAll('SELECT * FROM characters WHERE identifier = @identifier',
				{
					...
					if result[1].phone_number == numerek then
						MySQL.Async.execute('UPDATE characters SET phone_number = @phone_number WHERE identifier = @identifier',
						{
							['@identifier']   = currentUser,
							['@phone_number'] = nil
						})
