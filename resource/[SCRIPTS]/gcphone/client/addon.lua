...
RegisterNetEvent('Stinky-alert:callNumber')
AddEventHandler('Stinky-alert:callNumber', function(data)
  ...
  if message ~= nil and message ~= "" then
    TriggerServerEvent('Stinky-alert:startCall', number, message, {
      x = coords.x,
      y = coords.y,
      z = coords.z
...
RegisterNetEvent('Stinky-alert:callNumberD')
AddEventHandler('Stinky-alert:callNumberD', function(data)
  local playerPed   = PlayerPedId()
  local coords      = GetEntityCoords(playerPed)
...
RegisterNetEvent('Stinky-alert:accepted')
AddEventHandler('Stinky-alert:accepted', function()
  alert = nil	
  alertOwner = nil
...
function AcceptAlert(data, aO)
	...
	TriggerServerEvent('Stinky-alert:acceptedAlert', data, aO)
end

RegisterNetEvent('Stinky-alert:sendAlert')
AddEventHandler('Stinky-alert:sendAlert', function(data, id)
    ...
    if not HasStreamedTextureDictLoaded('ems') then
		...
		ESX.ShowAdvancedNotification('SAMS | Dyspozytornia', '~o~Zgłoszenie', data.message, 'ems', 2)
    ...
    if not HasStreamedTextureDictLoaded('mecano') then
		...
		ESX.ShowAdvancedNotification('LST | Dyspozytornia', '~o~Zgłoszenie', data.message, 'mecano', 2)
