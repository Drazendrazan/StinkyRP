...
Citizen.CreateThread(function()
	while true do
		...
		Ped.DisplayStreet = exports['StinkyRP']:DisplayingStreet()
...
local function CheckRadar(state)
	...
	if Ped.DisplayStreet ~= state then
		TriggerEvent('StinkySync:setDisplayStreet', state)
	end
end
...
Citizen.CreateThread(function()
    while true do
		...
		if Hide then
			DisplayRadar(false)
			TriggerEvent('StinkySync:setDisplayStreet', false)
		else
			if Ped.InVehicle then
				if not OnlyDriver or Ped.Driver then
					DisplayRadar(true)
					if not Ped.DisplayStreet then
						TriggerEvent('StinkySync:setDisplayStreet', true)
					end
				end
