...
Citizen.CreateThread(function()
	...
	while true do
		...
		heli = GetVehiclePedIsIn(lPed, false)
		
		local sleep = true

		local model = GetEntityModel(heli)
		if model == `ms_heli` or model == `pd_heli` then
			sleep = false
			if IsHeliHighEnough(heli) then
				if IsControlJustPressed(0, toggle_helicam) and (GetPedInVehicleSeat(heli, -1) == lPed or GetPedInVehicleSeat(heli, 0) == lPed) then -- Toggle Helicam
			...
			if IsControlJustPressed(0, toggle_display) and (GetPedInVehicleSeat(heli, -1) == lPed or GetPedInVehicleSeat(heli, 0) == lPed) then 
				ChangeDisplay()
			end

			if target_vehicle and (GetPedInVehicleSeat(heli, -1) == lPed or GetPedInVehicleSeat(heli, 0) == lPed) then
		...
		if helicam then
			...
			while helicam and not DecorExistOn(lPed, 'injured') and (GetVehiclePedIsIn(lPed, false) == heli) and IsHeliHighEnough(heli) do
				...
				if manual_spotlight then -- Continuously update manual spotlight direction, sync client-client with decorators
					...
					-- local netId = NetworkGetNetworkIdFromEntity(lPed)
		...
		if model == `pd_heli` and target_vehicle and not helicam and vehicle_display ~= 2 then
